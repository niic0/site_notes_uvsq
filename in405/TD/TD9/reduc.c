...
arg_t analyseArguments (int argc, char ** argv) {
	if (argc!=4){
		printf("bad usage of <%s> : $ %s m n opcode\n"
			   "n : sizeof tab"
			   "m : number of threads"
			   "opcode : operation to do", argv[0], argv[0]);
	}

	arg_t arg;
	arg.nbThreads = argv[1];
	arg.tailleTableau = argv[2];

	if(strcmp(argv[3], "+"))
		arg.code = OCD_SOMME;
	else if(strcmp(argv[3], "/"))
		arg.code = OCD_MOYENNE;
	else if(strcmp(argv[3], "M"))
		arg.code = OCD_MAX;
	else if(strcmp(argv[3], "m"))
		arg.code = OCD_MIN;
	else {
		printf("Wrong argument %s",argv[3]);
		exit(1);
	}
